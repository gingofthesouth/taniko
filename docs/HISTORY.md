...
## 8. Current state (10 August 2026)

Tāniko now conforms to its own contracts and ships an adoption kit. The repository carries a committed harness declaration (`.claude/harness.json`) and a shim (`bin/harness`) whose doctor and verify emit schema-conformant envelopes; the conformance suite judges the repo CONFORMANT, deny rules protect the gates from the agent they gate, and CI exercises the front door alongside the original three gates. The kit (`adoption/`) contains four phased implementation prompts (harness+loop, graph foundations, graph gates, first real run), five skills whose every rule carries the incident that earned it (§3–§4), and a POSIX installer for five agent tools, smoke-tested in CI across all six install modes. Adopting repositories consume the suite as a **pinned outside clone**: a `TANIKO_VERSION` file at their root, cloned by exact tag to `../taniko`, never inside the agent's sandbox. The v0.5 docket above remains frozen until real adopter runs produce strain evidence; the Needed/Offered/Implemented strain-notes channel built into every adoption prompt is how that evidence arrives.
